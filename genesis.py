...
def main():
  ...
  genesis_hash, nonce = generate_hash(block_header, net, options.nonce, options.bits)
...
def generate_hash(data_block, net, start_nonce, bits):
  ...
  while True:
    sha256_hash, header_hash = generate_hashes_from_block(data_block, net)
    last_updated             = calculate_hashrate(nonce, last_updated)
    if is_genesis_hash(header_hash, target):
      if net == "mainnet" or net == "testnet" or net == "regtest":
